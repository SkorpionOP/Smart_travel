...
function TravelInputForm({ onSubmit, loading, initialDestination, initialSpots }) {
  const [formData, setFormData] = useState(() => {
    ...
    const defaultData = { origin: '', destination: initialDestination || '', budget: '', travelers: 1, travel_style: 'Budget Friendly', duration: '', gemini_key: '', spots: initialSpots || [] };
    if (savedData) {
      try {
        const parsed = JSON.parse(savedData);
        if (initialDestination && parsed.destination !== initialDestination) {
          parsed.destination = initialDestination;
          parsed.spots = initialSpots || [];
        }
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} key={i} className="bg-[#F5F7FA] dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-white/5 flex flex-col justify-between hover:border-secondary hover:shadow-xl transition-all group">
            <div>
              ...
              <div className="flex items-center gap-2 text-yellow-500 mb-4 font-black">
                <Star size={20} fill="currentColor" /> {h.rating > 5 ? (h.rating / 2).toFixed(1) : h.rating} / 5.0
              </div>
              <p className="text-base text-gray-500 mb-2 flex items-center gap-2 font-medium"><MapPin size={18} className="text-gray-300" /> {h.distance_from_center} miles from center</p>
              {h.max_capacity && <p className="text-base text-gray-500 mb-6 flex items-center gap-2 font-medium"><Users size={18} className="text-secondary" /> Up to {h.max_capacity} Guests/Room</p>}
            </div>
...
function PgRecommendations({ pgs }) {
  if (!pgs || pgs.length === 0) return null;
  return (
    <section className="mb-10 p-10 bg-white dark:bg-gray-900 rounded-3xl shadow-lg border border-gray-50 dark:border-white/5">
      <h3 className="text-3xl font-bold flex items-center gap-3 mb-10 text-primary dark:text-white"><Star className="text-accent fill-accent" size={32} /> Budget Friendly PGs</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {pgs.map((h, i) => (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }} key={i} className="bg-[#F5F7FA] dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-white/5 flex flex-col justify-between hover:border-secondary hover:shadow-xl transition-all group">
            <div>
              ...
              <p className="text-base text-gray-500 mb-2 flex items-center gap-2 font-medium"><MapPin size={18} className="text-gray-300" /> {h.distance_from_center} miles from center</p>
              {h.max_capacity && <p className="text-base text-gray-500 mb-6 flex items-center gap-2 font-medium"><Users size={18} className="text-accent" /> Up to {h.max_capacity} Guests/Room</p>}
            </div>
            <div className="mt-auto font-black text-green-600 text-3xl border-t border-gray-100 pt-6 flex items-end">
              ₹{h.price} <span className="text-sm text-gray-400 font-semibold mb-2 ml-2">/ night</span>
            </div>
      ...
      <main className="max-w-6xl mx-auto px-6">
        ...
        {planData && !loading && (
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="space-y-12">
            ...
            {lastFormData?.travel_style === 'Budget Friendly' && <PgRecommendations pgs={planData.pgs} />}
